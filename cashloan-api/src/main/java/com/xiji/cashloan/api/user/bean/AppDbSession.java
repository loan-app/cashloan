...
public class AppDbSession {
	...
	public Object access(String token) {
		...
		if (rec == null) {
			...
			rec = dbService.queryRec("select * from cl_app_session where token=?", token);
			cache.put(token, rec);
		}
	...
	public AppSessionBean create(HttpServletRequest request, String loginname,
		...
		Map<String, Object> user = dbService.queryRec("select * from cl_user where login_name=?", loginname);
