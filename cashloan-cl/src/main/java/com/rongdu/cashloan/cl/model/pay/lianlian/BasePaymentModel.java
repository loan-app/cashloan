package com.rongdu.cashloan.cl.model.pay.lianlian;
...
import com.rongdu.cashloan.cl.model.pay.lianlian.util.SignUtil;
...
public class BasePaymentModel {
	...
	public BasePaymentModel() {
		super();
		this.setOid_partner(Global.getValue(LianLianConstant.BUSINESS_NO));
	...
	public boolean checkReturn(){
		if (StringUtil.isNotBlank(this.getRet_code()) && LianLianConstant.RESPONSE_SUCCESS_CODE.equals(this.getRet_code())) {
			return true;
		}
		return false;
	}
	...
	public boolean verfiySign(BasePaymentModel model) {
		...
		logger.debug("验签明文："+verifyJsonStr);
		return SignUtil.checksign(Global.getValue(LianLianConstant.PUBLIC_KEY), verifyJsonStr,
				model.getSign());
	}
	 ...
	 * @throws SystemRespException 
	 */
	public void response(String respMsg) throws UnsupportedEncodingException,
			IOException {
		...
		for (String name :  this.respParamNames()) {
			...
			Field f = fs.get(StringUtil.firstCharLowerCase(name));
