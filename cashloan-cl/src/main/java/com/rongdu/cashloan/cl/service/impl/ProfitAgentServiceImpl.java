...
import com.rongdu.cashloan.system.constant.SystemConstant;
	...
	public int freezeAgent(long userId,Date updateTime) {
		...
		if (StringUtil.isNotBlank(pa)) {
			...
			if (msg > 0) {
				...
				childrenMap.put("isUse",20);
				childrenMap.put("leaderId",pa.getUserId());
				profitAgentMapper.updateSelectiveChild(childrenMap);
