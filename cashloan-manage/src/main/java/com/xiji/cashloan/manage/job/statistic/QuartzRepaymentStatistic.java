...
@Component
@Lazy(value = false)
public class QuartzRepaymentStatistic implements Job {

    private static final Logger logger = Logger.getLogger(QuartzRepaymentStatistic.class);
     ...
     * @return
     */
    public String insertRepaymentStatistic(){
        logger.info("进入还款统计数据.....");
        RepaymentStatisticDataService repaymentStatisticDataService = (RepaymentStatisticDataService) BeanUtil.getBean("repaymentStatisticDataService");
        Date lateDate = repaymentStatisticDataService.getLateDate();
        ...
        List<RepaymentStatisticData> repaymentStatisticDataList;
        ...
        repaymentStatisticDataList = repaymentStatisticDataService.listRepaymentStatisticData(params);
        for(RepaymentStatisticData repaymentStatisticData :repaymentStatisticDataList ){
            repaymentStatisticData.setCreateTime(new Date());
            repaymentStatisticDataService.insert(repaymentStatisticData);
        }

        return "成功统计了"+repaymentStatisticDataList.size()+"天还款数据报表";
    }
    ...
    public void execute(JobExecutionContext context){
        ...
        // 查询当前任务信息
        QuartzInfo quartzInfo = quartzInfoService.findByCode("insertRepaymentStatistic");
        ...
        try {
            String remark = insertRepaymentStatistic();
