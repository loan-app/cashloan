  ...
  handleOk() {
        ...
        this.props.form.validateFields((errors, values) => {
          ...
          if(this.props.title=="新增"){       
            ...
            var url= '/modules/manage/rule/addRuleInfo.htm';
            var params=values;
          }   
         if(this.props.title=="编辑"){
             ...
             paramsData.detail.forEach(item=>{
                 this.props.dataTable.forEach(item1=>{
            ...
            var url= '/modules/manage/rule/modifyRuleInfo.htm';
          ...
          this.props.dataTable.forEach((item)=>{
              if(item.tableName==value){
                ...
                item.children.forEach((item,i)=>{
                     dataArray.push(<Option key={item.columnComment+i} value={item.columnName}>{item.columnComment}</Option>) ;
                  })    
              }
          })
                            ...
                            <Row>
                               <Col span="12">
                                    <FormItem {...formItemLayout} label="字段:">
                                     <Select style={{ width: '600px' }} optionFilterProp="children" disabled={props.title=="编辑"?props.canEdit:this.state.canEdit} multiple onSelect={this.select} {...getFieldProps('detail', {  initialValue:this.state.dataName ,rules: [{required:true,message:'必填',type:'array'}]})} > 
                                           {state.dataArray}
                                     </Select> 
                                    </FormItem>
                              </Col>
