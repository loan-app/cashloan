...
var SceneWin = React.createClass({
    ...
    handleOk(e) {
        ...
        this.props.form.validateFields((errors, values) => {
            var params = values;
            if(title == "新增"){
                url = "/modules/manage/rc/business/save.htm";
            }else{
                url = "/modules/manage/rc/business/update.htm";
                params.id = record.id;
            }
        ...
        return (
            <Modal title={props.title} visible={props.visible} onCancel={this.handleCancel} width="500" footer={modalBtns} >
                <Form horizontal form={this.props.form}>
                    <Input {...getFieldProps('statisticsId') } type="hidden" autoComplete="off" />
                    <Row>
                        <Col span="24">
                    ...
                    {/*<Row>
                        <Col span="24">
                            <FormItem  {...formItemLayout} label="扩展参数:"> 
                                <Input disabled={!props.canEdit}  {...getFieldProps('extend',{ rules: [{required:true,message:'必填'}]})} type="text"  /> 
                            </FormItem> 
                        </Col>
                    </Row>
                    <Row>
                        <Col span="24">
                            <FormItem  {...formItemLayout} label="请求地址:"> 
                                <Input disabled={!props.canEdit}  {...getFieldProps('url',{ rules: [{required:true,message:'必填'}]})} type="text"  /> 
                            </FormItem> 
                        </Col>
                    </Row>
                    <Row>
                        <Col span="24">
                            <FormItem  {...formItemLayout} label="测试地址:"> 
                                <Input disabled={!props.canEdit}  {...getFieldProps('testUrl',{ rules: [{required:true,message:'必填'}]})} type="text"  /> 
                            </FormItem> 
                        </Col>
                    </Row>*/}
                </Form>
