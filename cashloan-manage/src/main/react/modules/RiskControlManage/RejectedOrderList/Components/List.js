...
export default React.createClass({
    ...
    fetch(params = {}) {
        ...
        if(!params.searchParams){
            params.searchParams= JSON.stringify({state:'21'})
        }
        Utils.ajaxData({
            ...
            Utils.ajaxData({
                ...
                callback: (result) => {
                    if (result.code == 200) {
                        ...
                        this.refs.Lookdetails.refs.Tab1.setFieldsValue(dataForm);
                    }
                }
            });
            //console.log(record);
            record.state = title == "人工复审" ? "26" : record.state;
    ...
    render() {
        ...
        return (
            <div className="block-panel">
                ...
                />
                <Lookdetails ref="Lookdetails" key={state.keyModal} dataForm={state.dataForm} recordSoure={state.recordSoure} visible={state.visible} title={state.title} hideModal={me.hideModal} record={state.rowRecord}
