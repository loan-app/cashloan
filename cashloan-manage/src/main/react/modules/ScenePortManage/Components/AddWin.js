...
var AddWin = React.createClass({
    ...
    componentWillReceiveProps(nextProps, nextState) {
        if (this.state.first && nextProps.title && nextProps.title == '编辑') {
            var value1,value2;
            for(var i = 0; i < nextProps.ThirdPatryList.length; i++){
                if(nextProps.ThirdPatryList[i].props.value == nextProps.record.tppId){
                    value1 = nextProps.record.tppId;
                    break;
                }
                value1 = '';
            }
            for(var i = 0; i < nextProps.PortList.length; i++){
                if(nextProps.PortList[i].props.value == nextProps.record.businessId){
    ...
    handleOk(e) {
        ...
        this.props.form.validateFields((errors, values) => {
            ...
            if (title == "新增") {
                url = "/modules/manage/credit/sceneBusiness/save.htm";
                var data = values;
                data.type = me.state.value;
                data.tppName = me.state.value1;
                data.businessId = me.state.value2;
            } else if (title == "编辑") {
                url = "/modules/manage/credit/sceneBusiness/update.htm";
                var data = values;
                data.type = me.state.value;
                data.tppName = me.state.value1;
                data.businessId = me.state.value2;
    ...
    componentDidMount() {
        ...
        Utils.ajaxData({
            url: '/modules/manage/rc/statistics/listAll.htm',
            callback: (result) => {
                var data = result.data;
                data.forEach(item => {
    ...
    fetch(value){
        var SceneList1 = [];
        Utils.ajaxData({
            url: '/modules/manage/rc/business/listByStatisticsId.htm',
            data: {
                statisticsId: value
            },
            callback: (result) => {
    ...
    onChange1(value) {
     ...
     // 验证间隔天数
     isDay: function(rule, value, callback) {
        if (!value) {
            value = '';
        }
        
        if (!!value && value.match(/^\+?[1-9][0-9]*$/g) && value <=365 ) {  
            callback();
        } else {
            callback('请输入小于366的正整数');
        }
    },
                        ...
                        <Row>
                            <Col span="24">
                                <FormItem  {...formItemLayout} label="间隔时间（天）:">
                                    ...
                                    <Input type="text"  disabled={!props.canEdit}  {...getFieldProps('period', {
                                            rules: [
                                                { required: true, message: '必填' },
                                                { validator: this.isDay },
                                            ] 
                                         })} />         
                                </FormItem>
                            </Col>
