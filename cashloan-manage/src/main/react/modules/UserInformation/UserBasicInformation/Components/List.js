...
export default React.createClass({
  ...
  change(title,record){
    let me = this;
    let tip = title == '解除白名单' ? '是否确定解除' : '是否确定添加'
    confirm({
      title: tip,
      onOk: function () {
        Utils.ajaxData({
          url: "/modules/manage/user/updateWhiteState.htm",
          data: {
            id: record.id,
            state: title == '解除白名单' ? 0 : 20,
          },
          method: 'post',
          callback: (result) => {
            if (result.code == 200) {
  ...
  render() {
      ...
      render: (text, record) => {
        return <div>
          ...
          {record.state == "10" ? (
            ...
            <div>
          {record.state == "20" ? (
            <a href="javascript:;" onClick={me.change.bind(me, '解除白名单', record)}>解除白名单</a>
          ) : (
              <a href="javascript:;" onClick={me.change.bind(me, '添加白名单', record)}>添加白名单</a>
            )}
            </div>
    ...
    return (
      <div className="block-panel">
        ...
        <ReviewWin ref="ReviewWin" visible={state.visible} key={state.keyModal} recordSoure={state.recordSoure} title={state.title} hideModal={me.hideModal} record={state.record}
