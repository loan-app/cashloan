...
var AddRole = React.createClass({
  ...
  handleOk(e) {
    ...
    this.props.form.validateFields((errors, values) => {
      ...
      for (var i = 0; i < req.length; i++) {
        for (var item in req[i]) {
          if (
            this.state.type == "10" &&
            item.indexOf("id") == -1 &&
            item.indexOf("result") == -1
          ) {
            if (
              req[i][item] !== "" &&
              req[i][item] !== undefined &&
              req[i][item] !== null &&
              !reg.test(req[i][item])
            ) {
              continue;
      ...
      if (this.props.title == "编辑") {
        ...
        paramsData = {
          name: params.name,
          id: this.props.ruleData.rule.id,
          configList: JSON.stringify(params.configList),
          infoList: JSON.stringify(params.infoList),
          type: params.type
        };
        // //console.log(url)
      } else {
  ...
  transformeData(data) {
    ...
    data.forEach((item, index) => {
      ...
      formData[`${k}integral`] = Number(item.integral);
  ...
  transformeData2(data2) {
    ...
    data2.forEach((item, index) => {
      ...
      formData2[`${k}integral`] = Number(item.integral);
  ...
  render() {
    ...
    const formItem2 = getFieldValue("keys2").map(k => {
      return (
        <div key={k}>
          <Input
            type="hidden"
            {...getFieldProps(`${k}id2`, {
              initialValue: this.formData2 ? this.formData2[`${k}id2`] : 0
            })}
          />
          <Row>
            <Col span="8" className="col_h">
              ...
              >
                <Select
                  style={{ width: "100%" }}
                  placeholder="请选择"
                  {...getFieldProps(`${k}info_formula`, {
                    initialValue: this.formData2
                      ? this.formData2[`${k}formula`]
                      : ""
                  })}
                >
                  <Option value=">">></Option>
            ...
            <Col span="6" className="col_h">
              ...
              >
                <Input
                  type="number"
                  {...getFieldProps(`${k}info_integral`, {
                    onChange: this.changeInt,
                    getValueFromEvent: this.changeNUm,
                    initialValue: this.formData2
                      ? this.formData2[`${k}integral`]
                      : "",
                    rules: [
                      { type: "number", max: 2147483647, message: "输入过多" }
                    ]
                  })}
                />
              </FormItem>
            ...
            <Col span="6" className="col_h">
              <FormItem
                {...{ labelCol: {}, wrapperCol: { span: 20 } }}
                label=""
              >
                <Select
                  style={{ width: "100%" }}
                  placeholder="请选择"
                  {...getFieldProps(`${k}info_result`, {
                    initialValue: this.formData2
                      ? this.formData2[`${k}result`]
                      : ""
                  })}
                >
            ...
            <Col span="2">
              <Button className="ant-btn" icon="plus" onClick={this.add2} />
              <span style={{ padding: "0 5px" }} />
              {getFieldValue("keys2").length == 1 ? (
                ""
              ) : (
                <Button
                  className="ant-btn"
                  shape="circle"
                  icon="minus"
                  onClick={this.remove2.bind(this, k)}
                />
              )}
            </Col>
          </Row>
      ...
      return (
        <div key={k}>
          <Input
            type="hidden"
            {...getFieldProps(`${k}id`, {
              initialValue: this.formData ? this.formData[`${k}id`] : 0
            })}
          />
          <Row>
            <Col span="8" className="col_h">
              ...
              >
                <Cascader
                  options={this.state.tableInfo}
                  {...getFieldProps(`${k}ccolumn`, {
                    initialValue: this.formData
                      ? this.formData[`${k}ccolumn`]
                      : ""
                  })}
                  placeholder="请选择字段"
                />
            ...
            <Col span="4" className="col_h">
              ...
              >
                <Select
                  style={{ width: "100%" }}
                  placeholder="请选择"
                  {...getFieldProps(`${k}formula`, {
                    initialValue: this.formData
                      ? this.formData[`${k}formula`]
                      : "",
                ...
                >
                  ...
                  <Option value="include">包含</Option>
                  <Option value="exclude">不包含</Option>
                  <Option value="startWith">前X位包含</Option>
                  <Option value="endWith">后X位包含</Option>
                </Select>
              </FormItem>
            </Col>
            <Col span="3" className="col_h">
              ...
              >
                <Input
                  {...getFieldProps(`${k}cvalue`, {
                    initialValue: this.formData
                      ? this.formData[`${k}cvalue`]
                      : "",
                    rules: [{ max: 20, message: "输入过多" }]
                  })}
                  type="text"
                  autoComplete="off"
                />
            ...
            {this.state.type == "10" ? (
              <Col span="3" className="col_h">
                ...
                >
                  <Input
                    type="number"
                    {...getFieldProps(`${k}integral`, {
                      onChange: this.changeInt,
                      getValueFromEvent: this.changeNUm,
                      initialValue: this.formData
                        ? this.formData[`${k}integral`]
                        : "",
                      rules: [
                        { type: "number", max: 2147483647, message: "输入过多" }
                      ]
              ...
              <Col span="3" className="col_h">
                ...
                >
                  <Select
                    style={{ width: "100%" }}
                    placeholder="请选择"
                    {...getFieldProps(`${k}result`, {
                      initialValue: this.formData
                        ? this.formData[`${k}result`]
                        : ""
                    })}
                  >
                    {/*<Option  value="">结果选择</Option>*/}
            ...
            <Col span="2" className="col_h">
              <Button className="ant-btn" icon="plus" onClick={this.add} />
              <span style={{ padding: "0 5px" }} />
        ...
        <Form horizontal form={this.props.form}>
          <Row>
              ...
              >
                <Input
                  {...getFieldProps("name", {
                    initialValue: "",
                    rules: [
                      { required: true, message: "请输入规则名称，最多20个字!", max: 20 }
                    ]
                  })}
                  type="text"
                  autoComplete="off"
            ...
            </Col>
            {
              ...
              //       label="是否启用决策引擎:">
              //      <RadioGroup  {...getFieldProps("typeResultStatus",{initialValue:'10',onChange:this.onChange})}>
              //           <Radio value={"10"}>否</Radio>
              //           <Radio value={"20"}>是</Radio>
              //     </RadioGroup>
              //      {/*
              //        <Select style={{ width: '100%' }}  {...getFieldProps("typeResultStatus",{initialValue:"",onChange:this.onChange})} >
              //             <Option  value="">是否启用结果评级</Option>
          ...
          {
            //    this.state.typeResultStatus=="20"&& this.state.type=="10"?
            //   <div>
